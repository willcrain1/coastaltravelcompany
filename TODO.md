...
## 49. Cookie Consent & Cookie-Based Auth/Analytics

**Goal:** Implement a GDPR/CCPA-compliant cookie consent banner, migrate authentication from localStorage JWT to an HttpOnly cookie, and enable marketing analytics cookies gated behind explicit user consent.

### Legal requirements (prerequisite for any cookie deployment)

> You cannot place non-essential cookies (analytics, marketing) until the user has given affirmative consent. Essential cookies (auth session) are exempt from consent but must be disclosed in the privacy policy.

- [ ] Audit current storage usage — document every `localStorage` / `sessionStorage` / `document.cookie` call in `site/` and categorize as Essential vs. Analytics vs. Marketing
- [ ] Update `site/privacy.html` (item 39) to add a **Cookies** section: list each cookie by name, purpose, duration, and category
- [ ] Add "Do Not Sell or Share My Personal Information" anchor (`id="do-not-sell"`) to every page footer (CCPA requirement)

### Cookie consent banner

- [ ] Build `site/js/cookie-consent.js` — lightweight first-party consent manager (no external CDN dependency)
  - On first visit: show a fixed-bottom banner — "We use cookies for secure login and, with your consent, to understand how visitors use our site. [Accept All] [Essential Only] [Manage Preferences]"
  - Persist choice in `localStorage` under key `ctc_cookie_consent` as `{ essential: true, analytics: boolean, marketing: boolean, ts: ISO-string }`
  - On "Manage Preferences": show a modal with per-category toggles (Essential always on and disabled; Analytics; Marketing)
  - Re-show banner after 12 months or when consent record is missing/corrupt
  - Expose `window.CTC_Consent.hasAnalytics()` and `window.CTC_Consent.hasMarketing()` for use by analytics/marketing scripts
- [ ] Include `cookie-consent.js` in the `<head>` of every public HTML page (before any analytics or tracking script)
- [ ] Block GA4 / Clarity snippet execution until `CTC_Consent.hasAnalytics()` returns true — wrap existing snippets (items 32, 46) in a consent check

### HttpOnly auth cookie (replace localStorage JWT)

The Worker currently returns a raw JWT that the browser stores in `localStorage`. This migration moves the token into a server-set HttpOnly cookie so client-side JS can never read or exfiltrate it.

- [ ] **Worker — login endpoint** (`POST /auth/login`, `POST /auth/google/callback`): after verifying credentials and generating the JWT, add `Set-Cookie` response header:
  ```
  Set-Cookie: auth_token=<jwt>; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=604800
  ```
  Return `{ success: true }` in the body — do **not** return the raw JWT
- [ ] **Worker — auth middleware** (`worker/src/middleware/auth.js`): extract `auth_token` from the `Cookie` header instead of the `Authorization: Bearer` header; keep `Authorization` fallback for the Playwright test suite and API clients during transition
- [ ] **Worker — logout endpoint** (`POST /auth/logout`): clear the cookie with `Set-Cookie: auth_token=; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=0`
- [ ] **Worker — token refresh**: on each authenticated response, re-issue the cookie with a fresh `Max-Age` to implement a sliding session window
- [ ] **Frontend** (`site/js/auth.js`): remove all `localStorage.getItem('token')` / `localStorage.setItem('token', ...)` calls; remove the `Authorization` header injection from `fetch` wrappers — the browser sends the cookie automatically
- [ ] **CORS / SameSite consideration**: Worker is on a different subdomain from the static site; set `SameSite=None; Secure` if cross-site requests are needed, otherwise keep `SameSite=Lax`
- [ ] **D1 migration** (if implementing server-side session revocation): add `revoked_sessions` table with `(jti TEXT PRIMARY KEY, revoked_at INTEGER)`; check on each request; add `jti` claim to JWT

### Marketing analytics cookie

- [ ] On `CTC_Consent.hasMarketing() === true`, set a first-party analytics cookie via the Worker (`Set-Cookie: ctc_session=<uuid>; Secure; SameSite=Lax; Path=/; Max-Age=1800`) to track anonymous session continuity across pages
- [ ] Pass `ctc_session` value as a custom dimension in GA4 events so sessions can be stitched across pageviews
- [ ] Worker `POST /analytics/event` endpoint: accept `{ event, session_id, page, referrer }` — no IP or device fingerprint stored; rate-limit 60 events/session/minute via KV

### Acceptance criteria

- [ ] Banner appears on first visit; "Essential Only" blocks analytics scripts from firing (verify in browser DevTools Network tab — no GA4 requests)
- [ ] "Accept All" allows GA4 events and sets `ctc_session` cookie
- [ ] Login flow sets `auth_token` as HttpOnly (verify: cookie is present but `document.cookie` does not expose it in console)
- [ ] Authenticated API calls succeed with cookie-only auth (no `Authorization` header)
- [ ] Logout clears the `auth_token` cookie
- [ ] Playwright acceptance test: log in → assert `auth_token` cookie present → call protected endpoint → assert 200 → log out → assert protected endpoint returns 401
