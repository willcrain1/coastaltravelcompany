...
## ~~13. Individual Photo Purchase (Digital Licensing Store)~~ ✅ Done
...
### Admin curation
- [x] Add a "List for sale" toggle to each photo in `admin/store.html` — admin selects which photos from any gallery are available for purchase via a gallery photo browser
- [x] Per-photo store settings: title, brief description (location, collection context), and price per license tier
- [x] License tiers with separate pricing: **Personal use** (social media, personal prints), **Commercial digital** (website, advertising, digital marketing), **Commercial print** (brochures, signage, magazines), **Exclusive commercial** (removes photo from store after purchase, admin is notified)
- [x] Admin can set a photo as "featured" to appear prominently on the store front page
- [x] Store listing data saved in a `store_photos` D1 table; purchases in `photo_purchases` D1 table

### Storefront
- [x] Build `/shop.html` — public-facing photo store with masonry grid, watermarked preview thumbnails served via Worker proxy, cart, and Stripe checkout
- [x] Individual photo detail with license tier selector (radio buttons) and plain-English descriptions
- [x] Cart drawer allows purchasing multiple photos in one Stripe transaction
- [x] `admin/store.html` added to admin nav on all pages; "Store" link visible across all admin screens

### Purchase & delivery flow
- [x] Stripe Checkout session created by Worker (`POST /public/store/checkout`) — line items include photo title, license tier, and price
- [x] On payment success via webhook: purchase records written to D1, time-limited (72-hour) download tokens issued
- [x] Confirmation email via Resend: includes download links per photo, license type, and permitted uses
- [x] Admin notification email on each purchase
- [x] `GET /store/download/:token` streams full-resolution file from NAS through Worker (no R2 required)
- [x] `GET /verify/:licenseId` — public license lookup page; returns buyer, photo, license type, permitted uses
- [x] Exclusive purchases trigger admin notification and set photo status to `sold-exclusive`, removing it from storefront
- [x] `/shop-confirmation.html` — landing page from Stripe success redirect and from email download links
